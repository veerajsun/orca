...
export const Series: React.FC = () => {
  ...
  if (seriesData.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center text-gray-400 font-mono text-sm">
        No series added yet. Add some from the admin panel.
      </div>
    );
  }

  const heroSeries = seriesData.find((s) => s.id === 's-hero') || seriesData[0];
  const otherSeries = seriesData.filter((s) => s.id !== heroSeries.id);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12 animate-fade-in">
      
      {/* Featured Series Hero Banner matching Screenshot 3 */}
